"""
LEGACY API FILE - FOR REFERENCE ONLY

This file contains the COMPLETE old monolithic API structure that was replaced by the modular
src/api/ architecture. This is kept for reference during frontend migration and
should NOT be used in production.

The old API had all routes defined inline in a single file (1748 lines), with mixed business
logic and route handling. The new structure uses blueprints, controllers, and DTOs.

MIGRATION SUMMARY:
- OLD: Single src/api/__init__.py with all routes inline
- NEW: Modular src/api/ structure with:
  - src/api/routes/ (blueprint-based routing)
  - src/api/controllers/ (business logic separation)
  - src/api/dto/ (standardized response formats)
  - src/api/middleware/ (validation, error handling)

Key Changes:
1. Route definitions moved from inline to blueprint files
2. Business logic extracted to controllers
3. Response formats standardized with DTOs
4. Banking endpoints changed from /api/banks to /api/v2/banks
5. Enhanced filtering and pagination added

Migration Date: Backend refactor completed
Current Status: DEPRECATED - Use src/api/ structure instead
File Size: 1748 lines (vs ~46 lines in new structure)

This file is essential for:
- Understanding what the frontend expects
- Debugging migration issues
- Comparing old vs new API contracts
