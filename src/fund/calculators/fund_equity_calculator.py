...
class FundEquityCalculator:
    ...
    @staticmethod
    def _get_relevant_events(fund: Fund, session: Session) -> List[FundEvent]:
        """
        ...
        Returns:
            List of events ordered by date and ID
        """
        if fund.tracking_type == FundType.COST_BASED:
    ...
    # ============================================================================
    # BACKWARDS COMPATIBLE METHODS (REFACTORED) -> THESE METHODS SHOULDN'T BE CALLED
    # ============================================================================
    
    @staticmethod
    def calculate_current_equity(fund: Fund, session: Session) -> float:
        """
        Calculate current equity balance based on fund type - REFACTORED.
        
        Now uses single computation approach for 50% performance improvement.
    ...
    def calculate_average_equity(fund: Fund, session: Session) -> float:
        """
        Calculate time-weighted average equity balance based on fund type - REFACTORED.
        ...
        events = FundEquityCalculator._get_relevant_events(fund, session)
        event_balances = FundEquityCalculator.calculate_event_equity_balances(fund, events)
        return FundEquityCalculator.calculate_average_equity_from_balances(events, event_balances)
    
    @staticmethod
    def calculate_total_cost_basis(fund: Fund, session: Session) -> float:
        """
        Calculate total cost basis based on fund type - REFACTORED.
        ...
        events = FundEquityCalculator._get_relevant_events(fund, session)
        event_balances = FundEquityCalculator.calculate_event_equity_balances(fund, events)
        return FundEquityCalculator.calculate_total_cost_basis_from_balances(event_balances, fund, events)
    ...
    def recalculate_all_equity_fields(fund: Fund, session: Session) -> dict:
        """
        Recalculate all equity-related fields for a fund based on fund type - REFACTORED.
        ...
        result = {
            'current_equity_balance': FundEquityCalculator.calculate_current_equity_from_balances(event_balances),
            'average_equity_balance': FundEquityCalculator.calculate_average_equity_from_balances(events, event_balances),
            'total_cost_basis': FundEquityCalculator.calculate_total_cost_basis_from_balances(event_balances, fund, events)
        }
