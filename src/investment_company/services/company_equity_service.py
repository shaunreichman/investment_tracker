...
class CompanyEquityService:
    ...
    def update_company_equity_fields(self, company_id: int, fund_ids: List[int], session: Session) -> Optional[List[DomainFieldChange]]:
        """
        Update the equity fields of a company.
        """
        ...
        changes: list[DomainFieldChange] = []
        ...
        if old_average_equity_balance != average_equity_balance:
            changes.append(DomainFieldChange(domain_object_type=DomainObjectType.INVESTMENT_COMPANY, domain_object_id=company_id, field_name='average_equity_balance', old_value=old_average_equity_balance, new_value=average_equity_balance))
            company.average_equity_balance = average_equity_balance
        if old_current_equity_balance != current_equity_balance:
            changes.append(DomainFieldChange(domain_object_type=DomainObjectType.INVESTMENT_COMPANY, domain_object_id=company_id, field_name='current_equity_balance', old_value=old_current_equity_balance, new_value=current_equity_balance))
            company.current_equity_balance = current_equity_balance
        if old_end_date != last_event_date:
            changes.append(DomainFieldChange(domain_object_type=DomainObjectType.INVESTMENT_COMPANY, domain_object_id=company_id, field_name='end_date', old_value=old_end_date, new_value=last_event_date))
            ...
            if old_duration != new_duration:
                changes.append(DomainFieldChange(domain_object_type=DomainObjectType.INVESTMENT_COMPANY, domain_object_id=company_id, field_name='current_duration', old_value=old_duration, new_value=new_duration))
