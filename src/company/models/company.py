...
This module provides the Company model class representing companies which manage funds.
...
class Company(Base):
    """
    Model representing a company which manages funds.
    """
    __tablename__ = 'companies'
    
    id = Column(Integer, primary_key=True)  # (SYSTEM) auto-generated primary key
    name = Column(String(255), nullable=False)  # (MANUAL) company name
    ...
    __table_args__ = (
        ...
        Index('idx_companies_company_type', 'company_type'),
        Index('idx_companies_status', 'status'),
        # Composite indexes for common query patterns
        Index('idx_companies_type_status', 'company_type', 'status'),
        Index('idx_companies_name_status', 'name', 'status'),
    )
    
    def __repr__(self):
        return f"<Company(id={self.id}, name='{self.name}', company_type={self.company_type.value if self.company_type else None}, status={self.status.value})>"


    def get_field_classification(self) -> Dict[str, str]:
        """
        Field classification for the company model.
        
        Returns:
            Dict[str, str]: Field classification for the company model
        """
        return {
            'id': 'SYSTEM',
            'name': 'MANUAL',
