...
class FxRateFactory(SessionedFactory):
    """Factory for creating FxRate test data."""
    
    class Meta:
        model = FxRate

    from_currency = factory.LazyAttribute(lambda _: fake.random_element(elements=[
        'AUD', 'USD', 'EUR', 'GBP', 'CAD', 'NZD', 'SGD', 'HKD', 'JPY', 'CHF', 'CNY', 'KRW'
    ]))
    to_currency = factory.LazyAttribute(lambda _: fake.random_element(elements=[
        'AUD', 'USD', 'EUR', 'GBP', 'CAD', 'NZD', 'SGD', 'HKD', 'JPY', 'CHF', 'CNY', 'KRW'
    ]))
    date = factory.LazyAttribute(lambda _: fake.date_between(start_date='-2y', end_date='today'))
    fx_rate = factory.LazyAttribute(lambda _: fake.pyfloat(min_value=0.1, max_value=5.0, right_digits=4))
