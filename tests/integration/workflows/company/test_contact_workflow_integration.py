...
Tests cover:
xsw- Contact creation workflow with company validation
...
class TestContactWorkflowIntegration:
    ...
    def test_contact_creation_workflow_success(self, db_session):
        ...
        for factory in (CompanyFactory, ContactFactory):
        ...
        assert created_contact.company_id == company.id
        ...
        assert created_contact.company.id == company.id
        assert created_contact.company.name == company.name
    ...
    def test_contact_retrieval_workflow_by_company(self, db_session):
        ...
        company1 = CompanyFactory.create(name="Company Alpha")
        company2 = CompanyFactory.create(name="Company Beta")
        db_session.commit()
    ...
    def test_contact_retrieval_workflow_by_id(self, db_session):
        ...
        assert retrieved_contact.company_id == company.id
    ...
    def test_contact_workflow_database_relationship_integrity(self, db_session):
        """Test database relationship integrity between Contact and Company"""
        # Setup factories with session
        for factory in (CompanyFactory, ContactFactory):
            factory._meta.sqlalchemy_session = db_session
        
        # Create company
        company = CompanyFactory.create(name="Relationship Test Company")
        db_session.commit()
        ...
        for contact in all_contacts:
            ...
            assert contact.company.id == company.id
            assert contact.company.name == "Relationship Test Company"
            
            # Verify contact is in company's contacts (if back_populates is working)
            # Note: This depends on the Company model having a contacts relationship
    ...
    def test_contact_workflow_error_handling_invalid_sort_field(self, db_session):
        ...
        # Create company and contact
        company = CompanyFactory.create()
        ContactFactory.create(company=company)
        db_session.commit()
