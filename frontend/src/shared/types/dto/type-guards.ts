...
export function isApiResponse<T = any>(value: unknown): value is ApiResponse<T> {
  ...
  // Helper to check if details is valid (object/dict or null/undefined)
  const isValidDetails = (details: any): boolean => {
  ...
  return (
    ...
    isValidDetails(response.details)
  );
}
...
export function isControllerResponseDTO<T = any>(value: unknown): value is ControllerResponseDTO<T> {
  ...
  return (
    ...
    (dto.details === undefined || isValidDetails(dto.details))
  );
}
