...
import { Fund, FundEvent, FundTrackingType } from '@/fund/types';
import { formatCurrency, formatBrokerageFee, formatDate } from '@/shared/utils/formatters';
// event type label is now rendered via EventTypeChip
import { EventTypeChip } from '@/shared/ui/data-display/Chip';
...
export interface EventRowProps {
  ...
  onDeleteEvent: (event: FundEvent) => void;
}
...
const eventRowPropsAreEqual = (prevProps: EventRowProps, nextProps: EventRowProps): boolean => {
  ...
  if (
    ...
    // NOTE: Tax statement fields may not be present in FundEvent type
    (prevProps.event as any).interest_income_amount !== (nextProps.event as any).interest_income_amount ||
    (prevProps.event as any).interest_income_tax_rate !== (nextProps.event as any).interest_income_tax_rate ||
    (prevProps.event as any).dividend_franked_income_amount !== (nextProps.event as any).dividend_franked_income_amount ||
    (prevProps.event as any).dividend_franked_income_tax_rate !== (nextProps.event as any).dividend_franked_income_tax_rate ||
    (prevProps.event as any).dividend_unfranked_income_amount !== (nextProps.event as any).dividend_unfranked_income_amount ||
    (prevProps.event as any).dividend_unfranked_income_tax_rate !== (nextProps.event as any).dividend_unfranked_income_tax_rate ||
    (prevProps.event as any).capital_gain_income_amount !== (nextProps.event as any).capital_gain_income_amount ||
    (prevProps.event as any).capital_gain_income_tax_rate !== (nextProps.event as any).capital_gain_income_tax_rate ||
    (prevProps.event as any).eofy_debt_interest_deduction_sum_of_daily_interest !== (nextProps.event as any).eofy_debt_interest_deduction_sum_of_daily_interest ||
    (prevProps.event as any).eofy_debt_interest_deduction_rate !== (nextProps.event as any).eofy_debt_interest_deduction_rate ||
 ...
 */
const EquityCellContent: React.FC<{ event: FundEvent; fund: Fund }> = ({ 
  event, 
  fund 
}) => {
  const isNavBased = fund.tracking_type === FundTrackingType.NAV_BASED;
 ...
 * Extracted from FundDetail.tsx for reusability
 */
const NavUpdateCellContent: React.FC<{ event: FundEvent; fund: Fund }> = ({ 
  event, 
  fund 
}) => {
  return (
 ...
 * Extracted from FundDetail.tsx for reusability
 */
const TaxCellContent: React.FC<{ event: FundEvent; fund: Fund }> = ({ 
  event, 
  fund 
}) => {
    ...
    return (
      <Box>
        ...
        {(() => {
          ...
          // NOTE: Tax statement fields may not be present in FundEvent type
          // These are accessed via optional property access
          const eventWithTaxData = event as any;
          switch (event.tax_payment_type) {
            case 'EOFY_INTEREST_TAX':
              incomeAmount = eventWithTaxData.interest_income_amount ?? null;
              taxRate = eventWithTaxData.interest_income_tax_rate ?? null;
              break;
            case 'DIVIDENDS_FRANKED_TAX':
              incomeAmount = eventWithTaxData.dividend_franked_income_amount ?? null;
              taxRate = eventWithTaxData.dividend_franked_income_tax_rate ?? null;
              break;
            case 'DIVIDENDS_UNFRANKED_TAX':
              incomeAmount = eventWithTaxData.dividend_unfranked_income_amount ?? null;
              taxRate = eventWithTaxData.dividend_unfranked_income_tax_rate ?? null;
              break;
            case 'CAPITAL_GAINS_TAX':
              incomeAmount = eventWithTaxData.capital_gain_income_amount ?? null;
              taxRate = eventWithTaxData.capital_gain_income_tax_rate ?? null;
              break;
          }
    ...
    return (
      <Box>
        ...
        {(() => {
          ...
          // NOTE: EOFY debt cost fields may not be present in FundEvent type
          const eventWithTaxData = event as any;
          const totalInterest = eventWithTaxData.eofy_debt_interest_deduction_sum_of_daily_interest ?? null;
          const deductionRate = eventWithTaxData.eofy_debt_interest_deduction_rate ?? null;
