...
import { Fund, FundStatus } from '@/fund/types';
import { getStatusInfo } from '@/fund/utils/formatters/fundStatusFormatters';
import { StatusChip } from '@/shared/ui/data-display/Chip';
...
const FundDetailsSection: React.FC<SectionProps> = ({ fund, formatCurrency, formatDate, isLoading = false }) => {
  ...
  const fundDetails = [
    ...
    ...(fund.status === FundStatus.ACTIVE && fund.current_duration ? [{ label: 'Current Duration', value: `${fund.current_duration} months`, color: 'text.primary', icon: '⏱️', priority: 3 }] : [])
  ];
