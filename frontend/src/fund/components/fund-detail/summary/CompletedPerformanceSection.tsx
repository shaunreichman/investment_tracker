...
const CompletedPerformanceSection: React.FC<SectionProps> = ({ fund, formatCurrency, formatDate, isLoading = false }) => {
  ...
  const performanceMetrics = [
    {
      label: 'IRR',
      value: fund.completed_irr_gross,
      unit: '%',
      color: 'info.main',
      icon: '📊',
      priority: 1
    ...
    {
      label: 'Net-tax IRR',
      value: fund.completed_irr_after_tax,
      unit: '%',
      color: 'info.main',
      icon: '💰',
      priority: 2
          ...
          >
            ...
            >
              {metric.label === 'Actual Duration' 
                ...
                : `${metric.value !== null && metric.value !== undefined ? (metric.value * 100).toFixed(2) : '0.00'}${metric.unit}`}
            </Typography>
          </Box>
        ))}
      </Box>
