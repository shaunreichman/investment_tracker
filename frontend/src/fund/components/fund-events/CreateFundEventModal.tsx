...
import { useErrorHandler } from '@/shared/hooks/core';
import { useFund, useFundFinancialYears, useEventSubmission } from '@/fund/hooks';
import { calculateTaxPaymentDate } from '@/shared/utils/formatters';
import { useUnifiedForm, createValidator, validationRules } from '@/shared/hooks/forms';
import { SuccessBanner } from '@/shared/ui/feedback';
...
const CreateFundEventModal: React.FC<CreateFundEventModalProps> = ({
  ...
  const { financialYears, financialYearLabels, loading: financialYearsLoading } = useFundFinancialYears(fundId);
  const { handleSubmit: submitEvent, createFundEvent, createTaxStatement } = useEventSubmission({
    fundId,
    fundEntity: null, // Entity data not needed - use entity_id from fundData instead
        ...
        >
          ...
          {eventType === 'TAX_STATEMENT' && (
            <TaxStatementForm
              formData={formData as any}
              validationErrors={validationErrors as any}
              financialYears={financialYearLabels}
