 ...
 * 
 * Maps 1:1 to fundApi cash flow methods and backend cash flow endpoints
 * 
 * @module fund/hooks/useFundEventCashFlows
 */
...
export function useFundEventCashFlows(
  ...
  const queryFn = useCallback(
    () => fundApi.getFundEventCashFlows(params),
    [params]
  );
...
export function useFundEventCashFlowsByEvent(
  ...
  const queryFn = useCallback(
    () => fundApi.getFundEventCashFlowsByEvent(fundId, fundEventId),
    [fundId, fundEventId]
  );
...
export function useFundEventCashFlow(
  ...
  const queryFn = useCallback(
    () => fundApi.getFundEventCashFlow(fundId, fundEventId, fundEventCashFlowId),
...
export function useCreateFundEventCashFlow(fundId: number, fundEventId: number) {
  return useMutation<FundEventCashFlow, CreateFundEventCashFlowRequest>(
    (data) => fundApi.createFundEventCashFlow(fundId, fundEventId, data)
  );
}
...
export function useDeleteFundEventCashFlow(
  ...
  return useMutation<{ message: string; deleted_id: number }, void>(
    () => fundApi.deleteFundEventCashFlow(fundId, fundEventId, fundEventCashFlowId)
  );
}
