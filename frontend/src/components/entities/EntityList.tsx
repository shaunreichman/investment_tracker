...
import { useEntities, useDeleteEntity, parseEntityDependencyError, isEntityDependencyError } from '../../hooks/useEntitiesold';
...
export const EntityList: React.FC = () => {
  ...
  const handleDelete = async (entity: Entity) => {
    try {
      await deleteEntity.mutate(entity.id);
      ...
      // Other errors are handled by error handler in hook
    }
  };
          ...
          <CardContent sx={{ p: 0 }}>
            {viewMode === 'table' ? (
              <EntityTable 
                ...
                sortOrder={sortOrder}
                onSort={handleSort}
                isDeleting={deleteEntity.loading}
              />
            ) : (
              <EntityCards 
                entities={filteredAndSortedEntities}
