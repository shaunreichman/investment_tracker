 ...
 * 
 * Maps 1:1 to banking.api.ts bank methods and backend /api/banks endpoints
 * 
 * @module hooks/data/banking/useBanks
 */
...
export function useBanks(
  ...
  const queryFn = useCallback(() => api.banking.getBanks(params), [params]);
...
export function useBank(
  ...
  const queryFn = useCallback(
    () => api.banking.getBank(bankId, params),
    [bankId, params]
  );
...
export function useCreateBank() {
  return useMutation<Bank, CreateBankRequest>(
    (data) => api.banking.createBank(data)
  );
}
