 ...
 * 
 * @module hooks/data/banking/useBankAccounts
 */
...
export function useBankAccounts(
  ...
  const queryFn = useCallback(() => api.banking.getBankAccounts(params), [params]);
...
export function useBankAccountsByBankId(
  ...
  const queryFn = useCallback(
    () => api.banking.getBankAccountsByBankId(bankId, params),
    [bankId, params]
  );
...
export function useBankAccount(
  ...
  const queryFn = useCallback(
    () => api.banking.getBankAccount(bankId, bankAccountId, params),
...
export function useCreateBankAccount(bankId: number) {
  return useMutation<BankAccount, CreateBankAccountRequest>(
    (data) => api.banking.createBankAccount(bankId, data)
  );
}
...
export function useDeleteBankAccount(bankId: number, bankAccountId: number) {
  return useMutation<{ message: string; deleted_id: number }, void>(
    () => api.banking.deleteBankAccount(bankId, bankAccountId)
  );
}
