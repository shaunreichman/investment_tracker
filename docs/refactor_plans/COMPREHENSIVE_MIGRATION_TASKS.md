...
> 
> **New structure:**
> - Fund, tax, entity, rates, investment company: `src/<domain>/`
...
5. **SAFETY FIRST**: Keep old files active throughout entire migration, only comment out at the very end
...
- [x] **Task 1.1.1**: Move `with_session` decorator from `src/utils.py` to `src/shared/utils.py` (EXACT COPY)
- [x] **Task 1.1.2**: Update all imports to use `from ..shared.utils import with_session`
- [ ] **Task 1.1.3**: Comment out all lines in `src/utils.py` after ALL migrations complete (keep for reference)

#### 1.2 Shared Calculations
- [x] **Task 1.2.1**: Create `src/shared/calculations.py`
- [x] **Task 1.2.2**: Move shared calculation functions from `src/calculations.py` (EXACT COPIES):
  - [x] `get_equity_change_for_event(event, fund_type)` - Used by multiple modules
  - [x] `get_reconciliation_explanation(gross_diff, tax_diff, net_diff)` - Used by tax module
  - [x] `net_income(total_income, non_resident_withholding_tax_from_statement)` - Used by tax module
  - [x] `tax_payable(total_interest_income, interest_taxable_rate, non_resident_withholding_tax_from_statement)` - Used by tax module
  - [x] `interest_tax_benefit(total_interest_expense, interest_deduction_rate)` - Used by tax module
  - [x] `get_financial_year_dates(financial_year, tax_jurisdiction="AU")` - Used by tax module
- [x] **Task 1.2.3**: Update imports in all modules
...
- [x] **Task 2.2.1**: Create `src/fund/calculations.py`
- [x] **Task 2.2.2**: Move fund-specific calculation functions from `src/calculations.py` (EXACT COPIES):
  - [x] `calculate_irr(cash_flows, days_from_start, tolerance=1e-10, max_iterations=200)`
  - [x] `calculate_average_equity_balance_nav(unit_events, start_date, end_date)`
  - [x] `calculate_average_equity_balance_cost(capital_events)`
  - [x] `calculate_debt_cost(events, risk_free_rates, start_date, end_date, currency)`
  - [x] `calculate_nav_based_capital_gains(events)`
  - [x] `calculate_cost_based_capital_gains(events)`
  - [x] `orchestrate_nav_based_average_equity(unit_events)`
  - [x] `orchestrate_cost_based_average_equity(capital_events)`
  - [x] `orchestrate_irr_base(cash_flow_events, start_date, include_tax_payments=False, include_risk_free_charges=False, include_fy_debt_cost=False, return_cashflows=False)`
  - [x] `calculate_nav_event_amounts(unit_events)`
  - [x] `calculate_cumulative_units_and_cost_basis(unit_events, as_of_date=None)`
  - [x] `calculate_nav_based_cost_basis_for_irr(unit_events, as_of_date=None)`
- [x] **Task 2.2.3**: Update imports in `src/fund/models.py`

#### 2.3 Fund Creation (✅ EXISTS)
- [x] **Task 2.3.1**: Fund creation.py already exists in `src/fund/creation.py`
- [x] **Task 2.3.2**: Update imports in `src/fund/creation.py` to use new module structure
- [x] **Task 2.3.3**: Ensure creation methods work with migrated models and calculations
...
- [x] **Task 3.2.1**: Create `src/tax/calculations.py`
- [x] **Task 3.2.2**: Move tax-specific calculation functions from `src/calculations.py` (EXACT COPIES):
  - [x] `net_income(total_income, non_resident_withholding_tax_from_statement)`
  - [x] `tax_payable(total_interest_income, interest_taxable_rate, non_resident_withholding_tax_from_statement)`
  - [x] `interest_tax_benefit(total_interest_expense, interest_deduction_rate)`
- [x] **Task 3.2.3**: Update imports in `src/tax/models.py`

#### 3.3 Tax Creation
- [x] **Task 3.3.1**: Create `src/tax/creation.py`
- [x] **Task 3.3.2**: Move tax statement creation methods from `src/models.py` (EXACT COPIES):
  - [x] `_create_or_update_tax_statement_object(self, entity_id, financial_year, **kwargs)`
  - [x] `create_or_update_tax_statement(self, entity_id, financial_year, session=None, **kwargs)`
  - [x] `_create_tax_payment_event_object(self, tax_statement)`
  - [x] `create_tax_payment_events(self, session=None)`
- [x] **Task 3.3.3**: Update imports in `src/tax/models.py`
...
- [x] **Task 4.2.1**: Create `src/entity/calculations.py`
- [x] **Task 4.2.2**: Move entity-specific calculation functions from `src/calculations.py` (EXACT COPIES):
  - [x] `get_financial_years_for_fund_period(start_date, end_date, entity)`
  - [x] `get_financial_year_dates(financial_year, tax_jurisdiction="AU")` (already in shared)
- [x] **Task 4.2.3**: Update imports in `src/entity/models.py`

#### 4.3 Entity Creation
- [x] **Task 4.3.1**: Create `src/entity/creation.py`
- [x] **Task 4.3.2**: Create entity management methods
- [x] **Task 4.3.3**: Update imports in `src/entity/models.py`
...
- [x] **Task 5.2.1**: Create `src/rates/calculations.py`
- [x] **Task 5.2.2**: Move rate-specific calculation functions from `src/calculations.py` (EXACT COPIES):
  - [x] `get_risk_free_rate_for_date(target_date, risk_free_rates)`
- [x] **Task 5.2.3**: Update imports in `src/rates/models.py`

#### 5.3 Rates Creation
- [x] **Task 5.3.1**: Create `src/rates/creation.py`
- [x] **Task 5.3.2**: Create rate data management methods
- [x] **Task 5.3.3**: Update imports in `src/rates/models.py`

### 6. Investment Company Module Migration

#### 6.1 Investment Company Models
- [x] **Task 6.1.1**: Create `src/investment_company/models.py`
- [x] **Task 6.1.2**: Move InvestmentCompany class from `src/models.py` to `src/investment_company/models.py` (EXACT COPY)
- [x] **Task 6.1.3**: Update all imports and relationships

#### 6.2 Investment Company Calculations
- [x] **Task 6.2.1**: Create `src/investment_company/calculations.py`
- [x] **Task 6.2.2**: Move investment company-specific calculation functions (if any)
- [x] **Task 6.2.3**: Update imports in `src/investment_company/models.py`

#### 6.3 Investment Company Creation
- [x] **Task 6.3.1**: Create `src/investment_company/creation.py`
- [x] **Task 6.3.2**: Create investment company management methods
- [x] **Task 6.3.3**: Update imports in `src/investment_company/models.py`

### 7. Package Exports Migration

#### 7.1 Update Package __init__.py
- [x] **Task 7.1.1**: Update `src/__init__.py` to import from new module locations
- [x] **Task 7.1.2**: Update `__all__` list to reflect new structure
- [x] **Task 7.1.3**: Ensure backward compatibility for existing imports

### 8. Database Module

#### 8.1 Database Setup
- [x] **Task 8.1.1**: Review `src/database.py` for any needed updates
- [x] **Task 8.1.2**: Update imports if necessary
- [x] **Task 8.1.3**: Ensure database setup works with new module structure

### 9. Cleanup Tasks

#### 9.1 Comment Out Old Files (After ALL Migrations Complete)
...
- [ ] **Task 9.1.3**: Comment out all lines in `src/utils.py` after ALL migrations complete (keep for reference)
...
- ✅ Shared utilities migration (with_session decorator)
- ✅ Shared calculations migration (6 shared functions)

### Current Status
- **Total Tasks**: ~80+ tasks identified
- **Completed**: ~24 tasks (30%)
- **Remaining**: ~56 tasks (70%)
...
- **SAFETY FIRST**: Old files remain ACTIVE throughout entire migration process, only commented out at the very end
...
- Keep old files active during migration for maximum safety and rollback capability
